...
function startInstructions () { // Adds the instructions
...
function hangman() { // Adds hangman figure
  ...
  hangmanLink.setAttribute('href','https://www.vecteezy.com/free-vector/male-faces'); //Attribution
...
function initiateVariables(category) { // Gives value to important variables
  if (category === 'Colors') {
...
function wordGenerator() { // Adds word bubbles
...
function winAll() { // Called when user guesses all words in chosen category
  if (winner === 0) {
    removeWord();
    restartHangman();
    document.querySelector('.hangman-figure').setAttribute('src','img/trophy-gold.png');
    document.querySelector('.attribution').setAttribute('href','https://www.vecteezy.com/free-vector/winner');
    document.querySelector('.hangman-figure').setAttribute('alt','Winner Vectors by Vecteezy');
...
function clickedCube(identifier) { // Called when each key is clicked
  ...
  if (k === 7) { // When all the 6 attempts are used. Game over.
